    ...
    <>
      ...
      <section className="py-8 pb-24 px-4 bg-parish-white">
        <div className="max-w-4xl mx-auto relative">
          ...
          <div
            ...
            style={{
              background: "linear-gradient(to bottom, #3AA3FF, #FFB7E2)",
            }}
          />
            ...
            {timelineData.map((event, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Year bubble */}
                  <div
                    className="md:absolute md:left-1/2 md:-translate-x-1/2 z-10 w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0 shadow-lg text-white"
                    style={{
                      background: "linear-gradient(135deg, #3AA3FF, #FFB7E2)",
                    }}
                  >
                    <span className="font-serif text-xs font-bold text-center leading-tight px-1">
                      {event.year}
                    </span>
                  </div>
                  <div
      ...
      <section className="py-16 px-4" style={{ background: "#f0f8ff" }}>
        <div className="max-w-5xl mx-auto">
          <SectionHeader
            title={t("historyPage.gallery.title")}
          ...
          <p className="text-center text-gray-400 text-sm mt-6 font-body italic">
            {t("historyPage.gallery.editHint")}{" "}
            <code>src/pages/HistoryPage.tsx</code>
          </p>
        </div>
      </section>
    </>
...
function EventCard({ event }: { event: (typeof timelineData)[0] }) {
  ...
  return (
    <div className="bg-white border border-blue-100 rounded-xl p-5 shadow-sm hover:border-parish-blue hover:shadow-md transition-all">
      <h3 className="font-serif text-parish-blueDeep text-lg mb-1">{title}</h3>
      ...
      {event.imageUrl ? (
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, rgba(58,163,255,0.05), rgba(255,183,226,0.05))",
          }}
        >
          <span className="text-gray-300 text-xs font-body italic">
            {t("historyPage.noImage")}
          </span>
        </div>
      )}
    </div>
